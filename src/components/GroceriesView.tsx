...
const GroceriesView: React.FC<Props> = ({ groceries, setGroceries }) => {
  ...
  const addGrocery = () => {
    ...
    if (existing) {
      // If it exists, just ensure it's marked as "need" to buy it again
      setGroceries(prev => prev.map(g => g.id === existing.id ? { ...g, completed: false } : g));
    } else {
      setGroceries(prev => [{ id: Date.now().toString(), text: newGrocery, aisle: newAisle, completed: false }, ...prev]);
    }
    setNewGrocery('');
  };

  const toggleGrocery = (id: string) => {
    setGroceries(prev => prev.map(g => g.id === id ? { ...g, completed: !g.completed } : g));
  };

  const removeGrocery = (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    ...
    <div className="flex flex-col h-full max-w-4xl mx-auto">
      ...
      <div className="glass rounded-3xl p-6 flex gap-4 mb-8 shadow-xl">
        ...
        {newAisle === 'Other' ? (
           ...
           >
             <option value="" disabled>Select {newAisle} item to add...</option>
        ...
        {AISLES.map(aisle => {
          ...
          return (
            <div key={aisle} className="animate-in fade-in slide-in-from-bottom-2 duration-300">
              <h4 className="text-xs font-bold text-gray-500 uppercase tracking-[0.3em] mb-4 ml-2">{aisle}</h4>
                  ...
                  >
                    <div className="flex items-center gap-5">
                      {item.completed ? (
                        ...
                        <Circle className="text-gray-600 group-hover:text-gray-400 shrink-0 text-blue-500" size={28} />
                      )}
